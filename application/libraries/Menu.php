...
class menu {
  ...
  public function render(array $attrs = NULL, $current = NULL, array $items = NULL)
  {
    ...
    $menu .= '<ul'.($i == 1 ? self::attributes($attrs) : NULL).' class="menu desk" >';
    ...
    {
      $has_children = isset($item1['children']);
      $class = array();
      $has_children ? $class[] = 'parent' : NULL;
      if ( ! empty($current))
      {
        if ($current_class = self::current($current, $item1))
        {
          $class[] = $current_class;
        }
      }
    ...
    foreach ($items as $key => $item)
    {
      ...
      if (!$has_children) {
        $menu .= '<li><a href="'.$item['url'].'" class="topnav" >'.$item['title'].'</a>';
        ...
        $menu .= '<li><a href="'.$item['url'].'" class="topnav lis" >'.$item['title'].'</a>';
        $menu .= $has_children ? $this->render(NULL, $current, $item['children']) : NULL;
        $menu .= '</li>';
      }
	 ...
	 * @return  string  HTML unordered list
	 */
	public function render_tiny(array $attrs = NULL, $current = NULL, array $items = NULL)
	{
		static $i;
    ...
    //$menu = '<nav id="nav-wrap">
      //<ul'.($i == 1 ? self::attributes($attrs) : NULL).' id="nav">';
    ...
    if (!empty($items)) {
      ...
      if ($has_children) {
        ...
        $menu .= '<li '.$classes.'><a href="#">'.$item['title'].'</a>';
        $menu .= '</li>';
        $menu .= $has_children ? $this->render_tiny(NULL, $current, $item['children']) : NULL;
